...
def entropy(rows):
    ...
    for lbl in counts:
        prob_of_lbl= counts[lbl]/float(len(rows))
        entropy_val -= prob_of_lbl * math.log2(prob_of_lbl)
    #print("Entropy "+ str(entropy_val))
    return entropy_val
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        #print("Prediction Dictionary: "+ str(self.predictions))
        self.max_prediction_label = max(self.predictions, key = self.predictions.get)
        #print("Max Prediction: "+str(self.max_prediction_label))
        self.id = id
        self.depth = depth
        #print("ID of leaf node "+str(self.id))
        #print("Depth of leaf node "+str(self.depth))
...
def classify(row, node):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        return node.max_prediction_label
...
def print_tree(node, spacing=""):
    ...
    if isinstance(node, Leaf):
        ...
        print(spacing + "Predicted Class at Leaf Node: ",node.max_prediction_label)
        print(spacing + "Depth of Leaf Node: ",node.depth)
        print(spacing + "Id of Leaf Node: ", node.id)
        return

    # Print the question at this node
    print (spacing + "Question at Decision Node: "+str(node.question))
    print (spacing + "Depth of Desicion Node: "+str(node.depth ))
    print (spacing + "Id of Decision Node: " +str(node.id))
...
def getInnerNodes(node, innerNodes =[]):
    ...
    # Returns a list of all inner nodes of a tree
